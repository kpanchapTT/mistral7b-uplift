...
def main():
    ...
    (
        ...
        position_ids,
        prompts,
    ) = load_prompts_file(args, tokenizer)
...
def load_model_and_tokenizer(args):
    ...
    if args.flash_decode:
        assert (
            args.version == "efficient-40b"
        ), "Flash decode only supported for efficient-40b version"
        config.flash_decode = True

    if args.user_rows > 1:
        assert (
            args.version == "split-mq"
            or args.version == "efficient-40b"
            or args.version == "odkv"
            or args.version == "odkv_t"
        ), "User rows only supported for split-mq/efficient-40b/odkv version"

    # Load model
    if args.device in ["huggingface", "cuda"]:
        model = AutoModelForCausalLM.from_pretrained(
            args.model, config=config, trust_remote_code=True, cache_dir=args.hf_cache
        )
    else:
        if args.version == "tt" or args.version == "split-mq":
            ...
            if args.load_weights:
                ...
                model = RWForCausalLMTT.from_pretrained(
                    args.model, config=config, cache_dir=args.hf_cache
                )
            else:
...
def load_prompts_file(args, tokenizer):
    ...
    # Encode the prompt
    tokenized = tokenizer(
        prompts, padding="max_length", return_tensors="pt", max_length=args.seqlen
    )
    ...
    for _ in range(args.num_layers):
        past_key = torch.zeros(
            (args.user_rows, 8, args.seqlen, 64), dtype=torch.bfloat16
        )
...
def run_sync_efficient_40b(
    ...
    with reprint_output(
        output_type="list", initial_len=args.user_rows, interval=100
    ) as out:
        mlo = None
        ...
        args.profile_host_runtime = (
            True  # do this for now. TODO: add it as an argument to the script
        )
        ...
        attention_mask = torch.zeros(
            (1, args.user_rows, args.seqlen), dtype=torch.int32
        )
        ...
        while True:
            ...
            input_ids = (
                input_ids.squeeze().view(args.batch_size, args.user_rows).to(dest)
            )
            position_ids = (
                position_ids.squeeze().view(args.batch_size, args.user_rows).to(dest)
            )
            attention_mask = attention_mask.view(
                args.batch_size, args.user_rows, -1
            ).to(dest)

            end_pre = time()
            outputs = model.main_forward_part(
                input_ids,
            ...
            if args.profile_host_runtime:
                ...
                host_latencies["main_forward_part(blocks)"].append(
                    outputs[-1]["blocks"]
                )
                host_latencies["main_forward_part(post)"].append(outputs[-1]["post"])
                outputs = outputs[
                    :-1
                ]  # last output is the host runtime dictionary, ignore that

            start_final_fwd = time()
            if args.od_lm_head:
                output = (
                    outputs[0]
                    if not args.fracture_vocab
                    else torch.cat(outputs[: args.fracture_vocab_factor], dim=-1)
                )
                output = output.to("cpu").squeeze()
                past_key_values_cpy = (
                    outputs[1:]
                    if not args.fracture_vocab
                    else outputs[args.fracture_vocab_factor :]
                )
            else:
                assert (
                    args.fracture_vocab is False
                ), "Fractured vocab not supported for non-ODLMHead"
            ...
            if args.device == "pytorch":
                ...
                for i in range(args.num_layers):
                    new_past_key_values.append(
                        past_key_values_cpy[i * 16 : (i + 1) * 16]
                    )
                past_key_values = tuple(new_past_key_values)
            ...
            if not args.top_p:  # greedy
                ...
                token = top_pk_logits_efficient(
                    output.to(torch.float32), args.top_p, args.top_k, args.temperature
                )
            toppk_time = time() - toppk_time

            # Choose between prefill or decode tokens
            token = get_next_input(
                token, prompt_tokens, position_ids, prompt_token_counts
            )
            ...
            if args.print_option == "reprint":
                ...
                if mlo is None:
                    mlo = True
                    with open(
                        f"{args.device}_{args.version}_{args.num_layers}L_topp{args.top_p}_topk{args.top_k}_v0.txt",
                        "w",
                    ) as f:
            ...
            if args.profile_host_runtime:
                ...
                host_latencies["final_forward_part"].append(
                    end_final_fwd - start_final_fwd
                )
        ...
        sleep(5)
        print(
            f"User latency: {1000 * mean_latency:.1f} ms @ {1/mean_latency:.1f} tokens/s"
        )
        print(
        ...
        if args.profile_host_runtime:
            ...
            for k, v in host_latencies.items():
                if len(v) > warmup_batch:
                    print(
                        f"{k}: {1000 * sum(v[warmup_batch:]) / len(v[warmup_batch:]):.1f} ms"
                    )
