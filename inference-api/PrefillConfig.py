...
class PrefillConfig():
    ...
    def placement_overrides(self):
        pybuda = self.pybuda
        offset = 196
        [self.pybuda.config.internal_override_output_buffer_multiplier(f'concatenate_{188+i*offset}', multiplier=1) for i in range(len(self.pt_module.layers))]        
    ...
    def fracture_mlp_group(self, pybuda, fracture_mlp, chip_ids):
        ...
        for layer_num in range(len(self.pt_module.layers)):
            ...
            print(f"MLP Fracture: Layer: {layer_num}, matmul offset = {3+mlp_offset} & {8+mlp_offset}")
            ops = [f"matmul_{3+mlp_offset}", f"gelu_{5+mlp_offset}", f"matmul_{8+mlp_offset}"]
