...
class PyBudify(torch.nn.Module):
    def __init__(self, pt_module, device='silicon', arch='wormhole_b0', precision='fp32', amp_level=0, micro_batch_size=1, fuse=False, num_chips=1, perf=None, 
                 verify=False, log_level='ERROR', tti_save=None, tti_load=None, concurrent=False, netlist_name="pybudify_module", version='tt',
                 is_decode=True, opt_level=0, fracture_mlp=0, enable_tvm_cache=False, user_rows=1, num_tokens=1,
                 queues_on_host=False, od_lm_head=False, fracture_vocab=False, fracture_vocab_factor=8, flash_decode=False, num_outer_loops=1, **kwargs):
        ...
        if tti_load is not None:
            ...
            print(f"Model Loading Finished. Successfully Initialized in {loading_end_time-loading_start_time} seconds.")
        else:
    ...
    def ensure_initialized(self, *args):
        if not self.initialized and self.device != 'pytorch':
            ...
            print(f"Model Compilation Finished. Successfully Initialized in {compile_end_time-compile_start_time} seconds.")
            if self.concurrent:
        ...
        if self.device == 'pytorch':
            ...
            if self.version == 'efficient-40b':
                if not self.concurrent:
                    ...
                    if self.num_generated % (self.num_tokens//self.num_outer_loops) == 0:
                        self.pybuda.run_generate(input_count=self.num_tokens//self.num_outer_loops, write_index=0)
                ...
                if self.version == 'efficient-40b':
                    ...
                    self.tt0.push_to_inputs(args[:6])   # don't pass in kv over and over again
                ...
                outputs = tuple([ y.value() for y in ys if isinstance(y, self.pybuda.tensor.TensorFromPytorch)])
                if len(outputs) == 1:
                    outputs = outputs[0]
                if self.verify_cfg:
