...
# from decode_v0 import top_pk_logits_efficient, load_model_and_tokenizer
import json
import random
...
class DecodeBackend:
    ...
    class UserInfo:
        def __init__(self, user_id, prompt, position_id, params, tokenizer):
            ...
            tokenized = tokenizer(prompt, padding='max_length', return_tensors="pt", max_length=2048)
    ...
    def _init_kv_cache(self):
        ...
        for _ in range(self.num_layers):
            ...
            past_key = torch.zeros((self.max_users, 8, self.seqlen, 64), dtype=torch.bfloat16)
            past_value = torch.zeros((self.max_users, 8, self.seqlen, 64), dtype=torch.bfloat16)
    ...
    def _add_users_from_non_empty_queue(self, prompt_q):
        ...
        while not prompt_q.empty() and self._get_num_of_users() < self.max_users:
            ...
            user_info = DecodeBackend.UserInfo(user_id, prompt, 0, params, self.tokenizer)
            ...
            if self.verbose:
                with open('backend_logs/decode_backend.txt', 'a') as f:
                    f.write(f'Added user {user_id} to slot {idx} with prompt: {prompt}\n')
    ...
    def prepare_inputs(self):
        ...
        position_ids = [user_info.position_id if user_info is not None else 0 for user_info in self.users]
        ...
        attention_mask = torch.zeros((1, self.max_users, self.seqlen), dtype=torch.int32)

        for i, pos_id in enumerate(self.position_ids[0]):
            attention_mask[0, i, :pos_id] = 1

        kv_mask_id = self.position_ids.clone().view(1,-1).to(torch.int32) # batch, num_users
        ...
        prompt_tokens = [user_info.prompt_tokens if user_info is not None else torch.zeros((self.seqlen), dtype=torch.long) for user_info in self.users]
        prompt_lengths = [user_info.prompt_length if user_info is not None else 0 for user_info in self.users]
        ...
        if self.verbose:
            ...
            with open('backend_logs/decode_backend.txt', 'a') as f:
                ...
                f.write(f'user valid bitmask: {[1 if user is not None else 0 for user in self.users]}')


    def decode(self):
        # outputs = self.model.main_forward_part(self.input_ids, 
        ...
        # output = outputs[0] if not self.fracture_vocab else torch.cat(outputs[:self.fracture_vocab_factor], dim=-1)
        # output = output.to('cpu').squeeze()
        # past_key_values_cpy = outputs[1:] if not self.fracture_vocab else outputs[self.fracture_vocab_factor:]
        ...
        # else:
        #     output_tokens = top_pk_logits_efficient(output.to(torch.float32), self.top_p, self.top_k, self.temperature)
        # mock
        output_tokens = self.input_ids[-1].unsqueeze(0)
        # output_tokens = torch.full((1,32), self.tokenizer.eos_token_id)
        # print(output_tokens.shape)
        # print(output_tokens)

        # if user has hit max_length, send eos token
        for idx, user in enumerate(self.users):
            if user is not None and (user.position_id - user.prompt_length + 1) >= user.generation_params["max_tokens"]:
        ...
        if self.verbose:
            with open('backend_logs/decode_backend.txt', 'a') as f:
                ...
                f.write(f'decoded outputs: {[self.tokenizer.decode(input_id, clean_up_tokenization_spaces=True) for input_id in self.input_ids[0]]}\n')
    ...
    def push_outputs(self, output_q):
        for i, token in enumerate(self.input_ids[0]): # bc input_ids is 1x32
            ...
            if self.users[i].position_id == 0:
                ...
                first_token_text = self.tokenizer.decode(first_token, clean_up_tokenization_spaces=True)
            ...
            if self.verbose:
                with open('backend_logs/decode_backend.txt', 'a') as f:
                    f.write(f'\npush_outputs()\n')
                    f.write(f'pushing user_id: {self.users[i].user_id}, data: {return_text}\n')
    ...
    def send_status(self, prompt_q, status_q):
        if time() - self.cur_time > self.update_period:
            ...
            cur_status = (prompt_q.qsize(), self._get_num_of_users(), [user.user_id for user in self.users if user is not None])
