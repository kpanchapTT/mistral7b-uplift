...
class TT_functional:
    ...
    def flash_decode_attention(Q, Ks, Vs, attn_masks):
        ...
        global_max_m = torch.max(torch.cat(all_local_max_m, dim=-1), dim=-1, keepdim=True)[0]
        for V, ATT, local_max_m, local_row_sum_l in zip(Vs, all_local_attns, all_local_max_m, all_local_row_sum_l):
...
class Attention(nn.Module):
    ...
    def split_group_qkv_proj(self):
        ...
        self.wq_list = nn.ModuleList([Linear(self.hidden_size, group * self.head_dim, bias=self.bias) for _ in range(self.num_kv)])
        self.wk_list = nn.ModuleList([Linear(self.hidden_size, self.head_dim, bias=self.bias) for _ in range(self.num_kv)])
        self.wv_list = nn.ModuleList([Linear(self.hidden_size, self.head_dim, bias=self.bias) for _ in range(self.num_kv)])
        # TODO: load the weights from wq
        for i, module in enumerate(self.wq_list):
            module.weight = nn.Parameter(self.wq.weight[group*i*self.head_dim:group*(i+1)*self.head_dim,:].clone())
        for i, module in enumerate(self.wk_list):
            module.weight = nn.Parameter(self.wk.weight[i*self.head_dim:(i+1)*self.head_dim,:].clone())
        for i, module in enumerate(self.wv_list):
            module.weight = nn.Parameter(self.wv.weight[i*self.head_dim:(i+1)*self.head_dim,:].clone())

        self.dense_list = nn.ModuleList([Linear(self.hidden_size, self.hidden_size//8, bias=self.bias) for _ in range(self.num_kv)])
        for i, module in enumerate(self.dense_list):
            module.weight = nn.Parameter(self.dense.weight[:,i*self.hidden_size//8:(i+1)*self.hidden_size//8].clone())
    ...
    def forward(
        ...
        if self.efficient_40b:
            if self.flash_decode:
                return self.efficient_forward_flash_decode(hidden_states, cos, sin, attention_mask, layer_past, head_mask, output_attentions, kv_read_mask, kv_write_mask)
            else:
                return self.efficient_forward_split_group_qkv_proj(hidden_states, cos, sin, attention_mask, layer_past, head_mask, output_attentions, kv_read_mask, kv_write_mask)
        elif self.split_mq:
            if self.user_rows > 1:
                return self.split_mq_forward_users(hidden_states, cos, sin, attention_mask, layer_past, head_mask, output_attentions)
            else:
    ...
    def split_mq_forward_users(
        ...
        group = self.num_heads // self.num_kv
        attn_outputs = [TT_functional.scaled_dot_product_attention(
            query_layer[:, i*group:(i+1)*group,:,:].transpose(1, 2), # batch, users, group, head_dim TODO: try and remove this double transpose from above
            key_layer[:, i*users:(i+1)*users, :, :], # want batch, users, seq, head_dim
    ...
    def efficient_forward_split_group_qkv_proj(
        ...
        values = []
        for i in range(self.num_kv):
            query = self.wq_list[i](hidden_states).reshape(num_batch, users, -1, self.head_dim).transpose(1, 2) # [batch, group, users, head_dim]
            query = apply_rotary_pos_emb_q(query, cos, sin)
            queries.append(query.transpose(1, 2))

            key = self.wk_list[i](hidden_states).reshape(num_batch, 1, users, self.head_dim) # [batch, 1, users, head_dim]
            key = apply_rotary_pos_emb_k(key, cos, sin)
            keys.append(key.transpose(1, 2))

            value = self.wv_list[i](hidden_states).reshape(num_batch, users, 1, self.head_dim) # [batch, 1, users, head_dim]
            values.append(value)
    ...
    def efficient_forward_flash_decode(
        ...
        for i in range(self.num_kv):
            ...
            value = self.wv_list[i](hidden_states).reshape(num_batch, users, 1, self.head_dim) # [batch, users, 1, head_dim]
            values.append(value)
        ...
        # key_layer[:,i*users:(i+1)*users,:,:] : batch, users, seq, shead_dim
        group = self.num_heads // self.num_kv
        attn_outputs = [TT_functional.flash_decode_attention(
            Q=queries[i], 
        ...
        attn_outputs = [o.reshape(num_batch, users, -1) for o in attn_outputs] # Reshape at end of each MQA
        # attn_output = torch.cat(attn_outputs, dim=-1) # batch, 1, users, num_heads*head_dim

        # output_tensor = self.dense(attn_output)
        dense_outputs = [self.dense_list[i](attn_outputs[i]) for i in range(self.num_kv)]
        # Tree reduction
        tree_outputs = [dense_outputs[0]+dense_outputs[1], dense_outputs[2]+dense_outputs[3], dense_outputs[4]+dense_outputs[5], dense_outputs[6]+dense_outputs[7]]
        tree_outputs = [tree_outputs[0]+tree_outputs[1], tree_outputs[2]+tree_outputs[3]]
...
class SequentialCaller(nn.Module):
    ...
    def forward(self, hidden_states, cos, sin, attention_mask=None, *past_key_values):
        ...
        if len(past_key_values) > 0 and self.efficient_40b:
            ...
            kv_write_mask = self.kv_mask_embed(kv_mask_id).reshape(1,32,64,2048).transpose(-1, -2)
...
class RWModel(RWPreTrainedModel):
    def __init__(self, config: RWConfig):
        ...
        self.kv_mask_embed = nn.Embedding(2048, 2048) # this embedding should be an identity matrix
    ...
    def put_lm_head_on_device(self, lm_head):
        self.blocks = SequentialCaller(self.h, self.ln_f, lm_head, kv_mask_embed=self.kv_mask_embed)
        ...
        **deprecated_arguments,
        ...
        if attention_mask is not None:
            ...
            if self.batch_users:
                ...
                kv_mask_id = kv_mask_id.view(num_batch, users, 1).expand(num_batch, users, self.hidden_size // self.num_heads)
        ...
        if self.efficient_40b:
            ...
            outputs = self.blocks(hidden_states, cos, sin, attention_mask, *flattened_kv, kv_mask_id)
...
class LMHeadFractured(nn.Module):
    def __init__(self, lm_head, fracture_factor):
        ...
        self.lm_heads = nn.ModuleList([nn.Linear(embed_dim, fractured_vocab_size, bias=False) for _ in range(fracture_factor)])
        # load the weights
        for i in range(fracture_factor):
            self.lm_heads[i].weight.data = lm_head.weight.data[i*fractured_vocab_size:(i+1)*fractured_vocab_size,:]
...
class RWForCausalLM(RWPreTrainedModel):
    ...
    def fracture_vocab(self, fracture_factor):
        assert self.vocab_size % fracture_factor == 0, "vocab size must be divisible by fracture factor. try padding to vocab size"
